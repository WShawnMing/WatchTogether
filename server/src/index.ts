...
const ROOM_IDLE_TTL_MS =
  Number(process.env.ROOM_IDLE_TTL_MINUTES ?? '120') * 60 * 1000
...
function sanitizeNickname(value: string) {
  ...
  return trimmed ? trimmed.slice(0, 24) : `Viewer-${Math.floor(Math.random() * 90 + 10)}`
}
...
function emitRoomSnapshot(room: RoomState) {
  io.to(room.id).emit('room:snapshot', toRoomSnapshot(room))
}

function emitPlaybackState(room: RoomState, socketId?: string) {
...
function applyBuffering(room: RoomState) {
...
io.on('connection', (socket) => {
  socket.on(
    ...
    (payload: JoinRoomPayload, callback?: (result: JoinRoomResult) => void) => {
      ...
      emitRoomSnapshot(room)
      emitPlaybackState(room, socket.id)
    },
  )
...
setInterval(() => {
  ...
  for (const [roomId, room] of rooms.entries()) {
    if (room.members.size === 0 && now - room.lastActiveAt > ROOM_IDLE_TTL_MS) {
      deleteRoom(roomId)
    }
  }
}, 60_000)
...
httpServer.listen(port, () => {
  console.log(`watchtogether server listening on http://localhost:${port}`)
})
